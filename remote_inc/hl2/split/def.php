<?php $INCPATH="c:////hjemmesider//remote_inc";
...
<link rel="stylesheet" type="text/css" href="/hl2/i/style.css" />
<link rel="stylesheet" type="text/css" href="i/style.css" />
<meta charset="utf-8" />
...
{
	if(supports_html5_storage()){
		if(localStorage["mobile"]=="m"){
			window.location='/hl2/m/?IP='+escape(gup('IP'))+'&port='+escape(gup('port'))+'&do='+escape(gup('do'));
		}
		else
		{
			window.location='/hl2/?IP='+escape(gup('IP'))+'&port='+escape(gup('port'))+'&do='+escape(gup('do'));
		}
	}
}
function setV(k,v){
...
<p class="link right" onclick="setV('mobile','m');isMobile();">Testing mobile version</p>
